...
import { FileText, Play, FileOutput, Loader2 } from 'lucide-react';
...
export const WhatsAppParser: React.FC = () => {
  ...
  const handleParse = async () => {
    ...
    setMessages([]);
    try {
      const result = (await invoke(IPC_CHANNELS.WHATSAPP_PARSE_DB, {
        dbPath,
      })) as ChatMessage[];
      setMessages(result ?? []);
    } catch {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="WhatsApp Database Parser"
        description="Parse WhatsApp SQLite databases and generate forensic reports"
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="space-y-4">
          <FilePicker
            label="WhatsApp Database File"
            value={dbPath}
            onChange={setDbPath}
            placeholder="Select .db file..."
            filters={[{ name: 'SQLite Database', extensions: ['db', 'sqlite', 'crypt14', 'crypt12'] }]}
            disabled={parsing}
          />
          ...
          >
            ...
            {parsing ? 'Parsing Database...' : 'Parse Database'}
          </button>
            ...
            <div className="max-h-[500px] space-y-2 overflow-y-auto pr-2">
              {messages.length === 0 ? (
                <p className="text-sm text-slate-500">
                  No messages parsed yet. Select a database and click Parse.
                </p>
              ) : (
                messages.map((msg) => (
                  <div
