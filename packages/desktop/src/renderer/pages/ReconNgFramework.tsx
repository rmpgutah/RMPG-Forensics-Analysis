...
export const ReconNgFramework: React.FC = () => {
  const process = useProcess({
    channel: IPC_CHANNELS.RECONNG_RUN,
    progressChannel: IPC_CHANNELS.RECONNG_PROGRESS,
  });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Full-featured web reconnaissance framework with modular architecture (github.com/lanmaster53/recon-ng)"
        icon={<Terminal size={24} />}
      />
      <div className="grid grid-cols-2 gap-6">
        <div className="space-y-4">
          <div className="space-y-1.5">
            <label className="block text-sm font-medium text-slate-300">Target Domain or Entity</label>
            <input type="text" value={target} onChange={(e) => setTarget(e.target.value)} placeholder="Enter target domain or entity" disabled={process.isRunning} className="w-full rounded-md border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-white placeholder-slate-600 focus:border-blue-500 focus:outline-none disabled:opacity-50" />
          </div>
          ...
          <button onClick={handleStart} disabled={process.isRunning || !target || !outputFolder} className="flex w-full items-center justify-center gap-2 rounded-md bg-blue-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed">
            <Play size={16} />
            {process.isRunning ? 'Running...' : 'Start Recon'}
          </button>
        </div>
        <div className="space-y-4">
          ...
          <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-4">
            ...
            <ul className="space-y-1 text-xs text-slate-400">
              ...
              <li>• Workspace-based investigations</li>
              <li>• 80+ reconnaissance modules</li>
            </ul>
          </div>
        </div>
      </div>
